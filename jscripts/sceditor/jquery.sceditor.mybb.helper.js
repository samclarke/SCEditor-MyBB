(function($) {
	...
	if(typeof Thread !== 'undefined')
	{
		...
		Thread.quickReply = function(e) {
			var editor = $(textareaSelector).data('sceditor');
		...
		Thread.quickEditLoaded = function(request, pid)
		{
			...
			if(/<error>(.*)<\/error>/.test(responseText))
			{
				message = responseText.match(/<error>(.*)<\/error>/);

				alert('There was an error fetching the posts.\n\n'+(message[1] || "An unknown error occurred."));
			}
			else if(responseText)
			{
				$('#pid_'+pid).html(responseText);
		...
		Thread.quickEditSave = function(pid) {
			var editor = $('#quickedit_' + pid).data('sceditor');
		...
		Thread.multiQuotedLoaded = function(request)
		{
			...
			{
				editor = $(textareaSelector).sceditor('instance');
